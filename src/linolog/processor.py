...
class PrintProcessor:
    ...
    def _setup_agents(self):
        ...
        if Config.ENABLE_LLM:
            ...
            if llm_client.is_available():
                ...
                self.logger.warning(
                    "LLM client not available, using traditional agents"
                )
                llm_client = None
            ...
            for agent in self.agents:
                if agent.is_enabled():
                    try:
                        ...
                        self.logger.error(
                            f"Agent {agent.__class__.__name__} failed: {e}"
                        )
    ...
    def scan_existing_folders(self):
        ...
        if not os.path.exists(Config.WATCH_DIRECTORY):
            self.logger.warning(
                f"Watch directory does not exist: {Config.WATCH_DIRECTORY}"
            )
            return
    ...
    def _prompt_for_missing_fields(
        self, metadata: Dict[str, Any], missing_fields: List[str]
    ) -> Dict[str, Any]:
        ...
        for field in optional_fields:
            ...
            user_input = input(
                f"   Enter {display_name} (or press Enter to skip): "
            ).strip()
            if user_input:
                metadata[field] = user_input
