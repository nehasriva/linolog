...
class PrintFolderHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        self.logger.info(
            f"Event: on_created - {event.src_path} (is_dir: {event.is_directory})"
        )
        if event.is_directory:
            ...
            if self._is_valid_subfolder(event.src_path):
                ...
                self.logger.info(
                    f"Ignoring created folder (not valid subfolder): {event.src_path}"
                )

    def on_moved(self, event):
        """Called when a folder is moved/renamed."""
        self.logger.info(
            f"Event: on_moved - {event.src_path} -> {event.dest_path} (is_dir: {event.is_directory})"
        )
        if event.is_directory:
            ...
            if self._is_valid_subfolder(event.dest_path):
                ...
                self.logger.info(
                    f"Ignoring moved folder (not valid subfolder): {event.dest_path}"
                )

    def on_modified(self, event):
        """Called when a folder is modified (useful for copied folders)."""
        self.logger.info(
            f"Event: on_modified - {event.src_path} (is_dir: {event.is_directory})"
        )
        if event.is_directory:
            # Only process if it's a direct subfolder of the watch directory
            if self._is_valid_subfolder(event.src_path):
                ...
                self.logger.info(
                    f"Ignoring modified folder (not valid subfolder): {event.src_path}"
                )
    ...
    def _is_valid_subfolder(self, folder_path):
        ...
        try:
            ...
            is_valid = os.path.abspath(parent_dir) == os.path.abspath(
                self.watch_directory
            )
            self.logger.info(
                f"Subfolder check: {folder_path} -> parent: {parent_dir} -> valid: {is_valid}"
            )
            return is_valid
    ...
    def _process_folder(self, folder_path):
        ...
        try:
            ...
            try:
                ...
                if not items:
                    self.logger.info(
                        f"Folder appears empty, waiting longer: {folder_path}"
                    )
                    # Schedule another check in 3 seconds
                    timer = threading.Timer(
                        3.0, self._process_folder, args=[folder_path]
                    )
                    self.pending_folders[folder_path] = timer
                    timer.start()
                    return
